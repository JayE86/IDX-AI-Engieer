...
function parseMonths(question: string): number {
  const monthMatch = question.match(
    /\b(?:last|past|previous|over the last|over the past)\s+(\d+)\s+months?\b/i
  );

  if (monthMatch) {
    return Math.max(1, Number(monthMatch[1]));
  }
  ...
  const yearMatch = question.match(
    /\b(?:last|past|previous|over the last|over the past)\s+(\d+)\s+years?\b/i
  );

  if (yearMatch) {
    return Math.max(1, Number(yearMatch[1])) * 12;
  }
